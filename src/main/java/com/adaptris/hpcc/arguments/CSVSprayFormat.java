 ...
 * </table>
 *
 * <p>NOTE: Defaults are driven by dfplus command them selves, they will not be set unless explicitly set.</p>
 *
 * @author mwarman
 */
...
public class CSVSprayFormat extends SprayFormat {
  ...
  public void addCommandSpecificArguments(CommandLine cmdLine) {
    addArgumentIfNoNull(cmdLine, "encoding", encoding());
    addArgumentIfNoNull(cmdLine, "maxrecordsize", getMaxRecordSize());
    addArgumentIfNoNull(cmdLine, "separator", getSeparator());
    addArgumentIfNoNull(cmdLine, "terminator", getTerminator());
    addArgumentIfNoNull(cmdLine, "quote", getQuote());
  }
